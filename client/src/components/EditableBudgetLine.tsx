...
import { updateBudgetLine } from './../state/budgetLinesSlice'
...
const EditableBudgetLineElement: React.FC<EditableBudgetLineElementProps> = ({ budgetLine, newBalance, className }) => {
  ...
  return (
    <div className={`flex border-b border-gray-700 ${className}`}>
      ...
      <div
        className={`px-2 py-1 w-1/6 bg-white ${
          newBalance < 0 ? 'text-red-500' : 'text-green-900'
        } border-r border-gray-700`}>
        ${toDecimalPlacesString(newBalance, 2)} - {newBudgetLine.id}
      </div>
    </div>
  )
}
